    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      ...
      {isSuccess && (
        <div className="mb-4 rounded-md border border-green-200 bg-green-50 p-4" role="alert">
          <p className="text-green-800">{t('episode.episodeAdded')}</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {isSubmitting ? t('common.loading') : t('episode.addEpisode')}
        </button>
      </form>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          {t('episode.canAddMore').replace('{count}', String(remainingEpisodes))}
          {!hasLieEpisode && ` ${t('validation.episode.lieCount')}`}
        </p>
      </div>
    </div>
  );
